...
describe("Authentication and Authorization form", () => {
  ...
  test("User can successfully signUp", async () => {
    ...
    const res = await supertest(app).post("/api/signup").send(user);
    expect(res.status).toBe(200);
    expect(res.text).toBe(
      `User with this email leshya@mail.ru successfully created`
    );
  });
  ...
  test("User gets error on sign up with the same email", async () => {
    //@ts-ignore
    db.query = jest.fn(() =>
      Promise.reject(`User with this email already exists`)
    );
    const user = {
      email: "leshya@mail.ru",
      password: "hello",
        ...
        rows: [
          {
            ...
            isLogin: "true",
            password:
              "$2b$04$HjnkR.HRVpIf20EtYUVlweTW7W3qE7zNYtzhEWVeomhYT5toj7Gm6",
          },
        ],
      })
    );
  ...
  test("Refresh tokens become invalid on logout", async () => {
    const logoutRes = await supertest(app).post("/api/logout");
